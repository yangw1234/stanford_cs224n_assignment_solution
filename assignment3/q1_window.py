        ...
        for i in range(len(sentence)):
            ...
            for j in range(i - window_size, i + window_size + 1):
                if j < 0:
                    ...
                    feature += sentence[j]
            windowed_data.append((feature, label))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeddings = tf.nn.embedding_lookup(params=tf.Variable(self.pretrained_embeddings), ids=self.input_placeholder)
        shape = tf.shape(embeddings)
        embeddings = tf.reshape(embeddings, shape=[shape[0], shape[1]*shape[2]])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name="W", shape=(n_window_features*embed_size, hidden_size), initializer=tf.contrib.layers.xavier_initializer())
        b = tf.get_variable(name="b", shape=(hidden_size,), initializer=tf.zeros_initializer)
