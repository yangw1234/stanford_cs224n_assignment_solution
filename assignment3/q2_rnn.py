...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        new_sentence = sentence + [zero_vector] * pad_length
        new_label = labels + [zero_label] * pad_length
        mask = [True] * len(sentence) + [False] * pad_length
        ### END YOUR CODE ###
        ret.append((new_sentence[:max_length], new_label[:max_length], mask[:max_length]))
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, [shape[0], shape[1], shape[2]*shape[3]])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_dropout_t = tf.nn.dropout(o_t, dropout_rate)
