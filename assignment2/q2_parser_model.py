...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        words = tf.nn.embedding_lookup(table, self.input_placeholder)
        embeddings = tf.reshape(words, shape=[-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        W = tf.Variable(init(shape=(n_features*embed_size, hidden_size)))
        b1 = tf.Variable(tf.zeros(shape=(hidden_size,)))
        U = tf.Variable(init(shape=(hidden_size, n_classes)))
