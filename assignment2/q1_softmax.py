...
def softmax(x):
    ...
    shifted_x = x - tf.expand_dims(max, axis=1)
    exp_shifted_x = tf.exp(shifted_x)
    denominator = tf.expand_dims(tf.reduce_sum(exp_shifted_x, axis=1), axis=1)
    numerator = exp_shifted_x
...
def cross_entropy_loss(y, yhat):
    ...
    log_p = -tf.log(yhat)
    out = tf.reduce_sum(float_y * log_p)
    ### END YOUR CODE
